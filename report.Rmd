...
To overcome the aformentioned problems, I decided to use adopt a paradigm similar to one constructed by Mike Tuscherer known as "emerging strategies". (If you're interested, [here's his lecture on the topic](https://www.youtube.com/watch?v=WdGP120e4B0)). The plan to gain strength is as follows:

* 1.) Create a microcycle of training. My microcycle is one week long. It contains a training regimen that will be repeated multiple times. For example, if I perform the lowbar squat on Monday for 5x6@8 (5 sets of 6 reps at RPE8), highbar squat on Wednesday for 4x4@7, and the front squat on Friday for 4x8@8, I will repeat that protocol each week.
* 2.) *Establish a goal metric*. For myself, these metrics are the e1rm on the squat, bench press and deadlift.
* 3.) *Establish a way to monitor these metrics*. In this case, the e1rm were monitored by performing one set of one rep at RPE 8 on the target exercises one time per week. This roughly corresponds to 92% of an e1rm. 
* 4.) *Repeat the microcycle until the goal metric stagnates for n weeks in a row*, I selected 2. 
* 5.) If the program achieved the desired outcome, repeat the protocol. If not, then repeat steps 1-5. When reparameterizing the microcycle and perhaps the goal metric, use prior knowledge and the information gained in the microcycle. This is a vague description, because I'm not quite sure how to formalize this yet. In the future, I really want to update the parameters using a bayesian model, where the prior probability is the probability of obtaining sufficient results from a sequence of microcycles, and the updated parameter estimates are the parameters described in step one.  

# My implementation of emerging strategies

Let's look at how I parameterized my microcycle in the first block... 
I wanted to squat 13 sets per week, deadlift 8 sets per week , and bench press 17 sets per week. The average intensity was also fairly high, because my reps per ranged from 2 to 7. 
...
weeklyStatsAllBlocks$block<- as.factor(weeklyStatsAllBlocks$block)
pe1rm2<-ggplot(weeklyStatsAllBlocks,aes(x=week,y=e1rm)) + geom_line(aes(color=block))+geom_point(aes(color=block)) + facet_wrap(~exercise,scales = "free",ncol=2) + scale_y_continuous(limits = c(300,510),breaks=seq(300,500,by = 20)) +ggtitle("estimated one rep max (lbs) over time") +xlab("month")
#pe1rm2
...
plotly::ggplotly( ggplot(weekLevel %>% filter(group=="squatMovement"),aes(y=avgWeeklyIntensity,x=tWeek,color=as.factor(block)))+geom_line()+geom_point() + geom_hline(data = weekAvgs %>% filter(group=="squatMovement"),aes( yintercept=avgInt,color=as.factor(block)),linetype="dashed") )
...
plotly::ggplotly(
ggplot(weekLevel %>% filter(group=="squatMovement"),aes(y=numReps,x=tWeek,color=as.factor(block)))+geom_line()+geom_point() + geom_hline(data = weekAvgs %>% filter(group=="squatMovement"),aes( yintercept=meanReps,color=as.factor(block)),linetype="dashed") )
# + geom_hline(data = weekAvgs %>% filter(group=="squatMovement"),aes(yintercept=meanReps+sdReps,color=as.factor(block)),linetype="dotted") #+ geom_hline(data = weekAvgs %>% filter(group=="squatMovement"),aes(yintercept=meanReps-sdReps,color=as.factor(block)),linetype="dotted")
